...
export const home = async (req, res) => {
  try {
    const lands = await Land.find({}).sort({
      _id: -1
    });
    res.render("home", {
      pageTitle: "Home",
...
export const getContact = (req, res) => res.render("contact", {
  pageTitle: "Contact"
});
...
export const getAddLand = (req, res) => res.render("addLand", {
  pageTitle: "Add Land"
});
...
export const startSurvey = async (req, res) => {
  ...
  try {
    ...
    const phValue = phValues[phValues.length * Math.random() | 0]; // var COReadings = [[],[]];
    // var PHReadings = [[],[]];
    // var moistReadings = [[],[]];
    // for (i = 0; i < 2; i++) {
    //   for (j = 0; j < 4; j++) {
